...
menuTree.prototype={
	...
	editable:function(id){
		...
		function editItem(flag,aid,pid){//编辑菜单
			...
			var html = '<p><label>名称：</label><input class="menuName" type="text" value='+ tmpHtml + 
				' /></p><p><label>URL ：</label><input class="menuURL" type="text" value='+tmpData+' />'+isHideHtml+'</p>';
	...
	addChild:function(str,id,children,check,link){//生成单项item-nav
		...
		for(var i=0,l=children.length;i<l;i++){
			...
			str.push('<div class="item-nav'+(hasChild?' parent':'')+'" id="'+id+'_'+i+'">'+(check?'<span class="'+(children[i].checked?'onClick':'')+'"></span>':'')+'<a target="_self"' + (link?' href="'+children[i].url+'"':'') + attr + title + '>'+ children[i].name +'</a></div>');
